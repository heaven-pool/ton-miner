...
WIN_MINER = [f'{BASE_URL}/minertools-cuda-windows-x86-64.zip', f'{BASE_URL}/minertools-opencl-windows-x86-64.zip']

UBUNTU_1804_MINER = [f'{BASE_URL}/minertools-cuda-ubuntu-18.04-x86-64.tar.gz', f'{BASE_URL}/minertools-opencl-ubuntu-18.04-x86-64.tar.gz']
UBUNTU_2004_MINER = [f'{BASE_URL}/minertools-cuda-ubuntu-20.04-x86-64.tar.gz', f'{BASE_URL}/minertools-opencl-ubuntu-20.04-x86-64.tar.gz']

HIVE_BASE_URL = 'https://github.com/tontechio/pow-miner-gpu-hiveos/releases/download'
HIVEOS_MINER = [f'{HIVE_BASE_URL}/h.20211230.1/tonminer_opencl_hiveos_x86_64-h.20211230.1.tar.gz', f'{HIVE_BASE_URL}/h.20211230.1/tonminer_cuda_hiveos_x86_64-h.20211230.1.tar.gz']
...
def unzip_folder(src_folder: str, dst_folder: str):
    files = glob.glob(f"{src_folder}/*.zip") + glob.glob(f"{src_folder}/*.tar.gz")
...
for os_type in BUILD_PACK:
    ...
    [remove_patten_files(dst_folder,remove_patten) for remove_patten in REMOVE_PATTENS]
